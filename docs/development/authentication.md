...
- `check.RunE(*cobra.Command, []string) error`&mdash;testable entrypoint for `dr auth check`. Returns an error if authentication check fails. This is the preferred function to call in tests since it doesn't call `os.Exit()`.
- `check.Run(*cobra.Command, []string)`&mdash;wraps `RunE` and calls `os.Exit(1)` on error. Used as the command's `Run` handler in production.
